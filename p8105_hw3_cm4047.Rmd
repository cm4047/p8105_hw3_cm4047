...
From the scatterplot above, we can see that the average max temperature in January is lower than that in July in each station across years. Also, there is no significant increasing or decreasing trend from 1981 to 2010 in January and July.
```{r}
noaa_df %>% 
        filter(month %in% c("01", "07")) %>% 
        group_by(year, month, id) %>% 
...
```
From the max_min_bin_plot, we can see that most of the temperature for tmax is between 0 and 30 degrees C and most of the temperature for tmin is between 0 and 15 degrees C.  
